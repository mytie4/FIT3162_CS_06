...
import type { CreateClubDTO, UpdateClubDTO, Club } from "../entities/club.entity";
...
export async function updateClub(clubId: string, data: UpdateClubDTO, userId: string) {
  ...
  if (data.name !== undefined) {
    ...
    if (trimmed.length > 100) throw new ServiceError(400, "Club name should be 100 characters or less.");
    data.name = trimmed;
  }

  if (data.description !== undefined && data.description && data.description.length > 500) {
    throw new ServiceError(400, "Description should be 500 characters or less.");
  }
...
export async function updateMemberRole(clubId: string, targetUserId: string, newRole: string, requesterId: string) {
  ...
  if (!VALID_ROLES.includes(newRole as typeof VALID_ROLES[number])) {
    throw new ServiceError(400, `Invalid role. Must be one of: ${VALID_ROLES.join(', ')}`);
  }
  ...
  if (targetUserId === requesterId && newRole !== 'president') {
    throw new ServiceError(400, "You cannot demote yourself. Transfer presidency first.");
  }
...
export async function removeMember(clubId: string, targetUserId: string, requesterId: string) {
  ...
  if (targetUserId === requesterId) {
    throw new ServiceError(400, "You cannot remove yourself. Use leave club instead.");
  }
