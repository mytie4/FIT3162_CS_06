...
const MOCK_EVENTS = [
  { id: '1', title: 'Annual Tech Summit 2025', date: 'Nov 12-14, 2025', location: 'Caulfield Campus, Building H', status: 'in_progress', attendees: 124, color: '#3b82f6' },
  { id: '2', title: 'Code to Create Hackathon', date: 'Oct 20, 2025', location: 'Clayton Woodside Building', status: 'published', attendees: 85, color: '#14b8a6' },
]
...
export default function ClubDetailsPage() {
  ...
  const canManageEvents = currentUserRole === 'president' || currentUserRole === 'vice_president'
  ...
  if (isLoading) {
    return (
      <div className="club-details" style={{ padding: '64px 32px', textAlign: 'center', color: '#6b7280' }}>
        Loading club...
      </div>
    )
  }
  ...
  if (error || !club) {
    return (
      <div className="club-details" style={{ padding: '64px 32px', textAlign: 'center' }}>
        <p style={{ color: '#dc2626', marginBottom: '16px' }}>{error ?? 'Club not found'}</p>
    ...
    <div className="club-details">
      ...
      <div className="cd-container" style={{ marginTop: '-48px' }}>
          ...
          <div className="cd-header-right">
            ...
            {currentUserRole && (
              <>
                ...
                <button className="cd-leave-btn" onClick={() => setIsLeaveOpen(true)}>
        ...
        <div className="cd-tabs">
          {TABS.filter((tab) => tab !== 'Settings' || isPresident).map((tab) => (
            <button
              key={tab}
              ...
              <div>
                <div className="cd-card">
                  ...
                  <div className="cd-links">
                    {club.discord_link && (
                      <a href={club.discord_link} target="_blank" rel="noreferrer" className="cd-link-item">
                        <MessageSquare size={18} style={{ color: '#5865F2' }} /> Discord Server
                      </a>
                    )}
                    {club.instagram_link && (
                      <a href={club.instagram_link} target="_blank" rel="noreferrer" className="cd-link-item">
                        <LinkIcon size={18} style={{ color: '#E1306C' }} /> Instagram
                      </a>
                    )}
                    {club.website_link && (
                      <a href={club.website_link} target="_blank" rel="noreferrer" className="cd-link-item">
                        <Globe size={18} style={{ color: '#6b7280' }} /> Club Website
                      </a>
                    )}
                    {!club.discord_link && !club.instagram_link && !club.website_link && (
                      <p className="cd-no-links">No external links provided yet.</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              {MOCK_EVENTS.length > 0 ? (
                ...
                <div className="cd-events-empty">
                  ...
                  <h3>No events yet</h3>
                  <p>This club hasn't created any events. Create the first event to kick things off!</p>
                  {canManageEvents && (
                    <button className="cd-btn-primary">Create First Event</button>
                  )}
                </div>
              )}
            </div>
          ...
          {activeTab === 'Members' && (
            <div>
              <div className="cd-members-header">
                ...
                {canManageEvents && (
                  <button className="cd-btn-primary" onClick={() => setIsInviteOpen(true)}>
          ...
          {activeTab === 'Settings' && isPresident && (
            <div className="cd-settings">
              <div className="cd-settings-card">
                ...
                <div className="cd-settings-body">
                  <div className="cd-form-group">
                    ...
                    <input type="text" className="cd-form-input" defaultValue={club.name} />
                  </div>
                  <div className="cd-form-group">
                    <label className="cd-form-label">Club Category</label>
                    <select className="cd-form-select" defaultValue={club.type ?? ''}>
                  ...
                  <div className="cd-form-group">
                    <label className="cd-form-label">
                      Club Banner <span className="cd-form-label-hint">(Optional)</span>
                    </label>
                    <div className="cd-banner-preview" style={{ backgroundColor: bannerColor }}>
                  ...
                  <div className="cd-socials-section" style={{ paddingTop: '16px', borderTop: '1px solid #f3f4f6' }}>
                    <h4>External Links & Socials</h4>
                    <div className="cd-form-group" style={{ marginBottom: '16px' }}>
                      <label className="cd-form-label">
                        Shared Drive Link <span className="cd-form-label-hint">(Internal)</span>
                      </label>
                      <input type="url" className="cd-form-input" defaultValue={club.shared_drive_link ?? ''} placeholder="https://drive.google.com/..." />
                    </div>
                    <div className="cd-socials-grid">
                      <div className="cd-form-group">
                        <label className="cd-socials-label">
                          <MessageSquare size={14} style={{ color: '#5865F2' }} /> Discord Invite Link
                        </label>
                        <input type="url" className="cd-form-input" defaultValue={club.discord_link ?? ''} placeholder="https://discord.gg/..." />
                      </div>
                      <div className="cd-form-group">
                        <label className="cd-socials-label">
                          <LinkIcon size={14} style={{ color: '#E1306C' }} /> Instagram Link
                        </label>
                        <input type="url" className="cd-form-input" defaultValue={club.instagram_link ?? ''} placeholder="https://instagram.com/..." />
                      </div>
                    </div>
                    <div className="cd-form-group" style={{ marginTop: '16px' }}>
                      <label className="cd-socials-label">
                        <Globe size={14} style={{ color: '#6b7280' }} /> Website URL
                      </label>
                      <input type="url" className="cd-form-input" defaultValue={club.website_link ?? ''} placeholder="https://..." />
                    </div>
                  </div>
                </div>
              ...
              <div className="cd-danger-zone">
                <h3>Danger Zone</h3>
                <p>
                  Deleting a club is irreversible. All associated events, tasks, and member
